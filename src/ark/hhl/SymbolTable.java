...
if (params_matched)
 ...
 public int get_value(String name) {
  if (name.equals("HourGlass")) {
   return ((ark.bot.HourGlass) ? 1 : 0);
  } else if (name.equals("DraggingItem")) {
   return ((ark.bot.HaveDragItem()) ? 1 : 0);
  } else if (name.equals("Hungry")) {
   return ark.bot.Hungry;
  } else if (name.equals("HP")) {
   return ark.bot.HP;
  } else if (name.equals("Stamina")) {
   return ark.bot.Stamina;
  } else if (name.equals("PlayerID")) {
   return ark.bot.PlayerID;
  } else if (name.equals("Moving")) {
   if (ark.bot.mapview != null)
    return ((ark.bot.mapview.player_moving) ? 1 : 0);
   else
    return 0;
  } else if (name.equals("ContextMenuReady")) {
   return ((ark.bot.isFlowerMenuReady()) ? 1 : 0);
  } else if (name.equals("CraftReady")) {
   return ark.bot.is_craft_ready();
  } else if (name.equals("BuildReady")) {
   return (ark.bot.HaveBuildWindow())?1:0;
  } else {
   Variable v = get_variable(name);
   if (v != null) {
